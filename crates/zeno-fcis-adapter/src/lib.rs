//! Strict project-neutral callable and canonical JSON-line runtime adapters.
//!
//! JSON is a bounded shell interchange format only. Protocol meaning remains the
//! complete [`NormalizedDecision`] and its canonical component bytes. The same
//! adapter can mount Rust, Python, ESSO-generated, Morph-generated, or other
//! runtimes without granting transport authority over transition semantics.
...
impl ReplayFixture {
    ...
    /// Returns the model decision commitment.
    #[must_use]
    pub const fn model_hash(&self) -> Hash32 {
        self.model_hash
    }

    /// Returns the mounted runtime decision commitment.
    #[must_use]
    pub const fn runtime_hash(&self) -> Hash32 {
        self.runtime_hash
    }
...
mod tests {
    ...
    fn canonical_line_round_trips_exactly() {
        let decision = reject("policy_denied");
        ...
        assert_eq!(
            encode_decision_line(&decoded, JsonLimits::default())
    ...
    #[test]
    fn whitespace_and_unknown_fields_fail_closed() {
        let mut whitespace = encode_decision_line(&reject("reason"), JsonLimits::default())
            .unwrap_or_else(|error| panic!("encode: {error}"));
        whitespace.insert(1, b' ');
        assert!(matches!(
            decode_decision_line(&whitespace, JsonLimits::default()),
            Err(AdapterError::NonCanonicalJson)
        ));

        let canonical = encode_decision_line(&reject("reason"), JsonLimits::default())
            .unwrap_or_else(|error| panic!("encode: {error}"));
        let text = String::from_utf8(canonical).unwrap_or_else(|error| panic!("utf8: {error}"));
    ...
    fn uppercase_hex_fails_closed() {
        let canonical = encode_decision_line(&reject("reason"), JsonLimits::default())
            .unwrap_or_else(|error| panic!("encode: {error}"));
        let text = String::from_utf8(canonical).unwrap_or_else(|error| panic!("utf8: {error}"));
        let hostile = text.replacen("010101", "A10101", 1);
        assert!(matches!(
            decode_decision_line(hostile.as_bytes(), JsonLimits::default()),
            Err(AdapterError::InvalidHex) | Err(AdapterError::NonCanonicalJson)
        ));
    }
