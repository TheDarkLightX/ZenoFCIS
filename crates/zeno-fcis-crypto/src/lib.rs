...
use zeno_fcis_codec::{
    CommitmentHasher, Domain, Hash32, commitment,
};
...
const DOMAIN_DIGEST: Hash32 = Hash32::new([
    0x71, 0x1a, 0xf1, 0x6b, 0x1d, 0x6f, 0xbb, 0xbe, 0x7e, 0x6c, 0x01, 0x91, 0x0c, 0xd9,
    0x78, 0xdf, 0x19, 0x92, 0x4f, 0xd6, 0xf4, 0xf7, 0x81, 0x8f, 0x7f, 0x4e, 0x85, 0x81,
    0x35, 0x32, 0x72, 0xba,
]);
const MULTIBLOCK_MESSAGE: &[u8] =
...
pub fn verify_known_answers<H: CommitmentHasher>(
    ...
    let domain =
        Domain::new("zeno-fcis/test", 1).map_err(|_| ProviderVerificationError::SemanticEncoding)?;
    let observed = commitment::<H>(domain, b"abc")
