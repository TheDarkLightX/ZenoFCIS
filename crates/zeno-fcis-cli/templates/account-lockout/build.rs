...
fn main() {
    ...
    let reasons = project
        ...
        .map(|reason| {
            let disposition = match reason.id().get() {
                200..=202 => ReasonDisposition::Reject,
                203 => ReasonDisposition::CommittedFailure,
        ...
        .map(|channel| {
            ...
            ChannelDefinition::try_new(
                ...
                OperationSemantics::non_value(digest(
                    "example/account-lockout/channel",
                    b"Security alert; no value transfer",
                ))
                .expect("non-value channel"),
                profile::program_hash(),
            )
            .expect("reviewed alert channel")
        })
        .collect();
    ...
    .map(|(kind, raw, label)| {
        RegistryEntry::try_new(
            ...
            id(raw),
            name(label),
            digest("example/account-lockout/root", label.as_bytes()),
        )
        .expect("root entry")
    })
    .collect::<Vec<_>>();
    ...
    let project_profile = ProjectProfile::try_new(
        ...
        id(101),
        id(102),
        DomainPrefix::try_new("example/account-lockout").expect("domain"),
        ProfileBindings {
            schema_hash: schema
