...
pub fn source_hash() -> Hash32 {
    digest(
        "example/account-lockout/source",
        concat!(
...
pub fn program_hash() -> Hash32 {
    digest("example/account-lockout/program", source_hash().as_bytes())
}

pub fn checker_hash() -> Hash32 {
    digest("example/account-lockout/checker", source_hash().as_bytes())
}
...
pub fn manifest() -> LawManifest {
        ...
        .map(|kind| {
            if required.contains(&kind) {
                ...
                LawFamilyPolicy::not_applicable(
                    kind,
                    digest(
                        "example/account-lockout/non-value",
    ...
    let mut definitions = project
        ...
        .map(|law| {
            ...
            LawDefinition::try_new(
                ...
                scope,
                genesis,
                digest("example/account-lockout/law", &claim),
                checker_hash(),
    ...
    definitions.push(
        LawDefinition::try_new(
            ...
            no_genesis(),
            digest(
                "example/account-lockout/framework-law",
...
fn no_genesis() -> GenesisApplicability {
    GenesisApplicability::NotApplicable {
        rationale_hash: digest(
            "example/account-lockout/no-genesis",
