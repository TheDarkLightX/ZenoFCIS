//! Checks that the executed application decided a corpus of requests the
//! same way in every run this test makes.
//!
//! The corpus takes the strikes at 0 and 3, the identity tier at 0 and 3,
//! every region, the amount band at 0 and 4, and the counterparty risk at low
//! and high, with each screening requested by a customer and each
//! reinstatement requested by a customer and by a reviewer: 144 inputs.
//! `tests/conformance.rs` runs all 2,880 admitted inputs once.
//!
...
//! src/rules.rs synthesized/transition.rs` checks the decision code
...
fn corpus_digests(authority: &Authority, runs: u8) -> Vec<String> {
    ...
    let regions = [Region::Allowed, Region::Restricted, Region::Sanctioned];
    let risks = [CounterpartyRisk::Low, CounterpartyRisk::High];
    let requests = [
        (GatewayAction::Screen, false),
        (GatewayAction::Reinstate, false),
        (GatewayAction::Reinstate, true),
    ];
    let mut digests = Vec::new();
    for strikes in [0, 3] {
        for identity_tier in [0, 3] {
            for region in &regions {
                for amount_band in [0, 4] {
                    for risk in &risks {
                        for (action, reviewer) in requests.iter().cloned() {
                            // Each request has its own replay identity.
                            let input = format!(
                                "{strikes} {identity_tier} {region:?} {amount_band} {risk:?} {action:?} {reviewer}"
                            );
                            let root = project
                            ...
                            let witness = authority
                                .admit_invocation(
                                    ...
                                    profile::digest(
                                        "example/compliance-gateway/replay",
                                        input.as_bytes(),
                                    ),
                                )
                                .unwrap();
...
fn every_corpus_input_decides_identically_across_repeated_runs() {
    ...
    println!("determinism: 144 inputs x 8 runs in this process, 0 divergences");
}
...
fn decisions_match_in_child_processes_with_a_changed_environment() {
    ...
    println!("determinism: 144 inputs in 3 changed-environment processes, 0 divergences");
}
