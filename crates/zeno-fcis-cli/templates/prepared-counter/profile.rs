...
pub fn source_hash() -> Hash32 {
    digest(
        "example/prepared-counter/source",
        concat!(
...
pub fn program_hash() -> Hash32 {
    digest("example/prepared-counter/program", source_hash().as_bytes())
}

pub fn checker_hash() -> Hash32 {
    digest("example/prepared-counter/checker", source_hash().as_bytes())
}
        ...
        .map(|kind| {
            if required.contains(&kind) {
                ...
                LawFamilyPolicy::not_applicable(
                    kind,
                    digest(
                        "example/prepared-counter/non-value",
    ...
    let definitions = project
        ...
        .map(|law| {
            ...
            LawDefinition::try_new(
                ...
                scope,
                genesis,
                digest("example/prepared-counter/law", &claim),
                checker_hash(),
...
fn no_genesis() -> GenesisApplicability {
    GenesisApplicability::NotApplicable {
        rationale_hash: digest(
            "example/prepared-counter/no-genesis",
