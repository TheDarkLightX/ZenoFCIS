...
fn main() {
    ...
    let schema = lower_schema(
        ...
        vec![
            ...
            (stable(106), TypeKind::I128 { min: -1, max: 1 }),
        ],
        ...
        .map(|channel| {
            ...
            ChannelDefinition::try_new(
                ...
                OperationSemantics::non_value(digest(
                    "example/prepared-counter/channel",
    ...
    .map(|(kind, raw, label)| {
        RegistryEntry::try_new(
            ...
            id(raw),
            name(label),
            digest("example/prepared-counter/root", label.as_bytes()),
        )
        .expect("root entry")
    })
    .collect::<Vec<_>>();
    ...
    let project_profile = ProjectProfile::try_new(
        ...
        id(101),
        id(102),
        DomainPrefix::try_new("example/prepared-counter").expect("domain"),
        ProfileBindings {
            schema_hash: schema
            ...
            codec_hash: digest(
                "example/prepared-counter/codec",
