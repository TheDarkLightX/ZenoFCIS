...
impl LawEvidenceVerifier for NoExternalProofs {
    fn verifier_identity(&self) -> Hash32 {
        profile::digest(
            "example/prepared-counter/verifier",
...
fn observations(
    ...
    TraceStep::try_new(vec![
        ...
        observe(ProjectionRoot::Command, &[101, 120], command.first.0),
        observe(ProjectionRoot::Command, &[101, 121], command.second.0),
        observe(ProjectionRoot::Command, &[101, 122], command.third.0),
...
impl CounterLaws {
    fn check(
        ...
        LawObservation::try_new(
            profile::id(law_id),
            status,
            profile::digest("example/prepared-counter/observation", &witness),
        )
...
impl ProjectLawEngine for CounterLaws {
    fn evaluate(
        ...
        let (post, commit, outbox, relation, reason_ok) = match input.decision() {
            ...
            LawDecisionView::Reject { reason_id } => {
                let mut count = pre.count.0;
                let full = [command.first.0, command.second.0, command.third.0]
                    .into_iter()
                    .any(|delta| {
                        count += delta;
                        !(0..=3).contains(&count)
    ...
    fn evaluate_genesis(
        ...
        let step = observations(
            ...
            &state,
            &CounterCommand {
                first: CounterDelta(0),
                second: CounterDelta(0),
                third: CounterDelta(0),
            },
            &CounterContext(true),
        );
        let bytes = input
            .canonical_bytes()
            .map_err(|_| LawEngineFailure::InvalidOutput)?;
        Ok(vec![self.check(500, &step, state.count.0 == 0, &bytes)?])
    }
}
