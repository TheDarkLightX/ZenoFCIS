...
fn main() {
    ...
    let schema = lower_schema(
        ...
        vec![
            ...
            // At most three payment attempts; `project.zeno` states the limit.
            (stable(106), TypeKind::I128 { min: 0, max: 3 }),
            (
                stable(109),
                TypeKind::Text {
                    min_len: 1,
    ...
    let reasons = project
        ...
        .map(|reason| {
            let disposition = match reason.id().get() {
                ...
                204 => ReasonDisposition::CommittedFailure,
    ...
    let channels = project
        ...
        .iter()
        .map(|channel| {
            // Both requests ask another system to act. Payment moves money
            // in the provider's system, not in this state: the provider's
            // answer comes back as a command.
            let meaning: &[u8] = match channel.id().get() {
                300 => b"Payment capture or void request; no value moves in this state",
                301 => b"Shipping label request; no value transfer",
            ...
            ChannelDefinition::try_new(
                ...
                OperationSemantics::non_value(digest("example/order-fulfillment/channel", meaning))
    ...
    .map(|(kind, raw, label)| {
        RegistryEntry::try_new(
            ...
            id(raw),
            name(label),
            digest("example/order-fulfillment/root", label.as_bytes()),
        )
        .expect("root entry")
    })
    .collect::<Vec<_>>();
    ...
    let project_profile = ProjectProfile::try_new(
        ...
        id(101),
        id(102),
        DomainPrefix::try_new("example/order-fulfillment").expect("domain"),
        ProfileBindings {
            schema_hash: schema
            ...
            codec_hash: digest(
                "example/order-fulfillment/codec",
