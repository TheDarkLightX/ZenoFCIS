...
impl SolidityFungibleBinding {
    ...
    pub fn try_new(
        ...
        if capability_code == 0
            || token_address == [0_u8; 20]
            || runtime_code_hash == [0_u8; 32]
        {
...
pub fn generate_advanced_solidity(
    ...
    GeneratedOnchainBundle::try_new(
        ...
        vec![
            ...
            GeneratedOnchainFile::new(
                format!("solidity/{base}/MANIFEST.zfcis"),
                manifest,
            ),
            GeneratedOnchainFile::new(
                format!("solidity/{base}/AGENT_POLICY.md"),
                policy,
            ),
        ],
    )
...
fn render_contract(spec: &SolidityAdvancedSpec) -> Result<String, OnchainModelError> {
    ...
    writeln!(output, "/// @notice Generated by {SOLIDITY_ADVANCED_GENERATOR_ID}.")?;
    output.push_str(
    ...
    output.push_str("    error AssetBindingChanged(uint16 capability, bytes32 expected, bytes32 actual);\n\n");

    output.push_str("    event Initialized(bytes32 indexed stateHash, address indexed authority);\n");
...
fn render_execute(
    ...
    output.push_str("        Decision memory decision = _decide(beforeState, admittedCommand, context);\n");
    ...
    output.push_str("        }\n");
    output.push_str("        if (decision.reasonCode != 0) revert InvalidDecision(decision.reasonCode);\n");
...
fn render_typed_builders(
    ...
    for event in machine.events() {
        ...
        writeln!(output, "        planned.fieldCount = {};", event.fields().len())?;
    ...
    for capability in machine.capabilities() {
        ...
        writeln!(output, "        planned.capability = {};", capability.code())?;
        writeln!(
            output,
            "        planned.assetId = {};",
...
fn render_catalog_helpers(
    ...
    output.push_str("; }\n");
    output.push_str("    function _knownCapability(uint16 code) private pure returns (bool) { return ");
    ...
    output.push_str("    function _capabilityAsset(uint16 code) private pure returns (bytes32) {\n");
    for capability in machine.capabilities() {
        writeln!(
            output,
            "        if (code == {}) return {};",
    ...
    output.push_str("    function _capabilityMaxAmount(uint16 code) private pure returns (uint128) {\n");
    for capability in machine.capabilities() {
        writeln!(
            output,
            "        if (code == {}) return {};",
    ...
    output.push_str("    function _capabilityMaxUses(uint16 code) private pure returns (uint8) {\n");
    for capability in machine.capabilities() {
        writeln!(
            output,
            "        if (code == {}) return {};",
