...
use zeno_fcis_codegen::{
    ...
    SolanaAnchorSpec, SolanaFungibleBinding, SolidityAdvancedSpec, SolidityFungibleBinding,
    generate_advanced_solidity, generate_solana_anchor,
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let solidity = SolidityAdvancedSpec::try_new(
        machine.clone(),
        vec![SolidityFungibleBinding::try_new(
            7,
            [3_u8; 20],
            [4_u8; 32],
        )?],
...
) -> Result<OnchainField, Box<dyn Error>> {
...
fn retained_fixture_readme(machine_hash: zeno_fcis_codegen::Hash32) -> String {
    format!(
