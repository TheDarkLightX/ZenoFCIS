...
// demonstration: the twelve requests that `journey` in the template's
...
export const template = {
  ...
  context: [
    { name: "identity_tier", label: "Identity tier (0 unverified through 3)", kind: "integer", min: 0, max: 3, initial: 2 },
  ...
  commands: [
    {
      ...
      fields: [
        ...
        { name: "amount_band", label: "Amount band (0 under 100, 1 under 1,000, 2 under 10,000, 3 under 50,000, 4 above)", kind: "integer", min: 0, max: 4, initial: 1 },
        { name: "counterparty_risk", label: "Counterparty risk", kind: "choice", options: [["Low", "Low"], ["Medium", "Medium"], ["High", "High"]], initial: "Low" },
      ],
    },
    { name: "Reinstate", label: "Reinstate the account", fields: [] },
  ],
  ...
  describe: (request) => request.command === "Screen"
    ? `Screen ${request.region}, band ${request.amount_band}, ${request.counterparty_risk} risk, tier ${request.identity_tier}${request.reviewer ? ", reviewer" : ""}`
    : `Reinstate, tier ${request.identity_tier}${request.reviewer ? ", reviewer" : ""}`,
  demonstration: [
