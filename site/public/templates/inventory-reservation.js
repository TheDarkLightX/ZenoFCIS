...
// scripted demonstration: the eleven commands that `journey` in the
// template's src/lib.rs makes, with the decision each expects. The
// interrupted delivery and the database reopen that follow them need the
// SQLite shell and are not part of the page.

const quantity = { name: "quantity", label: "Quantity (1 to 3)", kind: "integer", min: 1, max: 3, initial: 1 };
...
export const template = {
  ...
  help: "Say whether the operator is authorized, then move stock: each button sends one command with its quantity through the authority.",
  context: [
  ...
  describe: (request) => `${request.command} ${request.quantity}${request.authorized ? "" : ", not authorized"}`,
  demonstration: [
