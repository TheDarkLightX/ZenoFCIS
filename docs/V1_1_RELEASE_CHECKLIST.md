# ZenoFCIS V1.1 release checklist

This is the owner runbook for the stable `1.1.0` release. It
...
The [2026-09-12 pre-release assessment](V1_RELEASE_STATUS.md) records the
qualification gaps before stable release preparation. Its dated observations do not replace this checklist.

The generated-application receipt must also run the unchanged V1.0 consumer
against the V1.1 crate archives. The retained protocol source baseline and
embedded canonical tests must be unchanged.

## Release identities

For V1.1, all of these values must agree:

```text
Cargo workspace version: 1.1.0
...
- [ ] Merge the reviewed release stack onto `main` without a semantic rebase or
...
- [ ] Confirm `Cargo.toml`, `release/package-set.toml`, `CHANGELOG.md`,
      `docs/V1_1_RELEASE_NOTES.md`, and this checklist name `1.1.0`.
...
python3 tools/rc_package.py build --output /tmp/zeno-fcis-v11
(cd /tmp/zeno-fcis-v11 && sha256sum --check SHA256SUMS)
```
...
- [ ] `PACKAGED-APPLICATION.json` reports the exact source commit, clean checker
      and archive sources, admitted package/dependency identities, and passing durable-counter and prepared-counter
      applications generated by the CLI built from those package contents.
...
## C. Create the signed stable tag

Tagging is an owner action performed only after sections A and B pass.

```bash
git tag --sign --annotate v1.1.0 <exact-commit> \
  --message "ZenoFCIS 1.1.0"
...
- [ ] Confirm the read-only `release-candidate` workflow ran from
      `refs/tags/v1.1.0` and passed at that exact commit.
...
- [ ] Do not move, delete, or recreate a published tag to repair a failure.
      Correct source is released under a new version.
...
- [ ] After each command, confirm crates.io serves exactly version
      `1.1.0` and wait until dependencies are visible before continuing.
- [ ] Record the crates.io response and published package checksum.
- [ ] Publish `zeno-fcis` last.
- [ ] Run the checked external-consumer project against crates.io without a
...
```bash
v1_smoke_dir="$(mktemp -d)"
cp -R test-projects/external-consumer/. "$v1_smoke_dir/"
sed -i '/^[[:space:]]*path = /d' "$v1_smoke_dir/Cargo.toml"
rm "$v1_smoke_dir/Cargo.lock"
cargo +1.97.1 generate-lockfile --manifest-path "$v1_smoke_dir/Cargo.toml"
cargo +1.97.1 check --manifest-path "$v1_smoke_dir/Cargo.toml" --locked
```

Inspect the generated lockfile and confirm every ZenoFCIS crate was resolved
from crates.io at `1.1.0` before deleting the disposable directory.
...
3. do not reuse `1.1.0` for changed source;
4. yank an affected crate version when it is unsafe or unusable, while
   preserving the public incident record;
5. repair and publish the complete coherent set under a new version.
