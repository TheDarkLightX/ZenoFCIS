...
The generic builder retains this low-level interface. Bootstrap-generated projects expose the narrower path documented in `GENERATED_CATALOG_TRANSITION.md` and `GENERATED_COMMAND_CONTEXT_ENVELOPES.md`: a private reconstructed catalog plus schema-admitted root, command, and context witnesses, with no caller-supplied raw catalog, state `Value`, command hash, or context hash.
