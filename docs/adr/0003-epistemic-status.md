...
| `ToolRunStatus::Undefined` | The claim has no value at this observation assignment: an overflow, a division by zero, or an inexact exact division | Checked | Replayed like `Refuted`. Evaluation is strict, so the claim does not hold there. |
| `ProposedUnsat` for an inductive claim (`ObligationScope::InductiveStepOverLaws`) | Every transition that satisfies the assumed laws preserves the invariant | Attested | cvc5's `unsat`, and the SMT encoding, which a pinned differential test checks against exhaustive replay. The result holds of the application only with the base case on its exact genesis state, and with each assumed law enforced on the decisions it is assumed on. The application checks both. |
