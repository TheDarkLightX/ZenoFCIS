...
- a private-field `GeneratedProject` that reconstructs the exact catalog, admits generated root/command/context types, derives command/context commitments, and starts transitions only from those witnesses;
...
- the generated transition entry point accepts no caller-supplied catalog, raw state value, or raw command/context commitment;
