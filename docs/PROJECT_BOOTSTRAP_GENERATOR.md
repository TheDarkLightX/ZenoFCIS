...
- a private-field `GeneratedProject` that reconstructs the exact catalog, admits generated root/command/context types, derives command/context commitments, and returns a private-inner transition wrapper with typed direct root-field read/update, context-observation, reason, effect, and channel methods and no raw path mutation or context-observation surface;
...
Generated smart constructors enforce Rust-level payload/destination type selection. `GeneratedProject::try_new` reconstructs the complete catalog, revalidates every cross-binding, and compares its exact schema, profile, catalog, and provider identities with generation-time constants. `admit_root` converts only the generated typed root and admits it under the catalog provider, including when the nested schema-codegen provider differs. `admit_command` and `admit_context` admit only the generated profile types and derive commitments over their complete schema-bound envelope bytes under visible, role-separated domains. The transition method accepts those three witnesses and returns a private-inner wrapper. Direct record-root field read methods fix existing generated paths and return the selected generated field types; update methods fix those paths and accept only those types. Non-record roots receive no direct-field methods. Record contexts receive one fixed observation method per reviewed field, while non-record contexts receive one fixed whole-root observation method. Its `require` and `fail_if` methods accept separate reason types copied from the catalog's existing dispositions. Per-effect and per-channel methods fix the exact stable ID and schema-generated value types; absent and exact commitments are fixed by generation, present commitments require `NonZeroHash`, and only `Any` commitments remain arbitrary `Hash32` inputs. Final catalog validation remains authoritative for successor schema admission, reason disposition, authority and subject commitments, aggregate limits, and project policy.
...
- generated context-observation methods fix the profile namespace and reviewed schema path while exposing no raw `AccessPath` parameter;
