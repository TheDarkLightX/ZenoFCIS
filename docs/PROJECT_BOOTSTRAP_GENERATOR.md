...
- a private-field `GeneratedProject` that reconstructs the exact catalog, admits generated root/command/context types, derives command/context commitments, and returns a private-inner transition wrapper with typed direct root-field update, reason, effect, and channel methods;
...
- generated direct root-field update methods fix existing schema paths and types, while the high-level wrapper exposes no raw update path/value pair;
