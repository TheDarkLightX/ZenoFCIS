...
holds on every state the application commits.
...
The durable-counter template's `tests/induction.rs` runs both application
checks against a real shell and its manifest.
...
- genesis satisfies it, by (b) and (f);
...
- A counter whose accept law is `post == pre + amount` and whose every-commit
  ...
  - `pre * pre >= 0` is `undefined (overflow)` near √(i128::MAX). The
    ...
    defined.
- The same counter with a committed-failure law `post == 0`: `pre >= 0` is
  proved with `accept [401] failure [403]`. Without the `failure` group, a
  committed failure is unconstrained, and the counterexample carries
  `zeno_decision_kind 2`.
- The durable counter: `counters_never_negative` is proved from its action
  laws alone. `pre.count <= 3` is refuted at 3 + 1 = 4, which shows the cap
...
- A proof of the step alone says nothing about the application. The base case
  and the assumption check must also pass in the application's own tests.
...
- A proved invariant is only as meaningful as the laws it assumes. Laws that
