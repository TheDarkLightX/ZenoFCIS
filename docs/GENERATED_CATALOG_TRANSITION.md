...
The project bootstrap generator emits an exact `GeneratedProject` binding between the reviewed `ProjectCatalog`, the schema-generated root envelope, and `CataloguedTransitionBuilder`. The generated type owns a private reconstructed catalog. Callers cannot start this generated transition path with an arbitrary catalog or a raw `Value`.
...
Successful startup returns the existing `CataloguedTransitionBuilder`. Sealing and three-way decision semantics are unchanged.
