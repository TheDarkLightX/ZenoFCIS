...
This is the owner runbook for the stable `1.0.0` release. It
...
For V1, all of these values must agree:

```text
Cargo workspace version: 1.0.0
package-set version:      1.0.0
Git tag:                  v1.0.0
...
- [ ] Confirm `Cargo.toml`, `release/package-set.toml`, `CHANGELOG.md`,
      `docs/V1_RELEASE_NOTES.md`, and this checklist name `1.0.0`.
...
(cd /tmp/zeno-fcis-v1 && sha256sum --check SHA256SUMS)
```
...
- [ ] `PACKAGED-APPLICATION.json` reports the exact source commit, clean checker
      and archive sources, admitted package/dependency identities, and a passing
      application generated by the CLI built from those package contents.
...
```bash
git tag --sign --annotate v1.0.0 <exact-commit> \
  --message "ZenoFCIS 1.0.0"
...
- [ ] Confirm the read-only `release-candidate` workflow ran from
      `refs/tags/v1.0.0` and passed at that exact commit.
...
- [ ] After each command, confirm crates.io serves exactly version
      `1.0.0` and wait until dependencies are visible before continuing.
...
from crates.io at `1.0.0` before deleting the disposable directory.
...
`v1.0.0` tag already exists, select a new version; never retarget `v1.0.0`. A
...
3. do not reuse `1.0.0` for changed source;
...
Stable `1.0.0` requires a separate reviewed commit and tag from RC3:
...
Cargo API stability begins at `1.0.0`. Existing canonical protocol
